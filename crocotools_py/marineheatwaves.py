...
def _compute_stratification_arrays(ds_temp, ds_temp_daily, ds_zeta_daily, target_dates,
    ...
    for i in range(0, n_eta, batch_size):
        ...
        levs = np.apply_along_axis(
            lambda col: np.searchsorted(col, target_z), axis=1,
            arr=z_slice.transpose(0, 2, 3, 1).reshape(-1, z_slice.shape[1])
        ).reshape(z_slice.shape[0], z_slice.shape[2], z_slice.shape[3])
        levs = np.clip(levs, 1, num_levels - 1)
 
        temp_at_target = np.full((z_slice.shape[0], z_slice.shape[2], z_slice.shape[3]), np.nan, dtype='float32')
        salt_at_target = np.full_like(temp_at_target, np.nan)
        for tt in range(z_slice.shape[0]):
            for ii in range(z_slice.shape[2]):
                for jj in range(z_slice.shape[3]):
                    lev = levs[tt, ii, jj]
                    z_up, z_down = z_slice[tt, lev, ii, jj], z_slice[tt, lev - 1, ii, jj]
                    if z_up == z_down:
                        continue
                    frac = (target_z - z_down) / (z_up - z_down)
                    temp_at_target[tt, ii, jj] = (temp_slice[tt, lev - 1, ii, jj]
                                                   + frac * (temp_slice[tt, lev, ii, jj] - temp_slice[tt, lev - 1, ii, jj]))
                    salt_at_target[tt, ii, jj] = (salt_slice[tt, lev - 1, ii, jj]
                                                   + frac * (salt_slice[tt, lev, ii, jj] - salt_slice[tt, lev - 1, ii, jj]))
...
def plot_flag_map(site_data, today, start_date, end_date, out_path, lat, lon, depth_name="Surface"):
    ...
    ax.add_feature(cfeature.LAND, facecolor="white", edgecolor='black', zorder=2); ax.add_feature(cfeature.COASTLINE, linewidth=0.8, edgecolor="#555544", zorder=5)
...
def animate_surface_anomalies(cat_ds, lat, lon, out_path):
    ...
    ax.add_feature(cfeature.COASTLINE, linewidth=0.6); ax.add_feature(cfeature.LAND, facecolor="white", edgecolor='black', zorder=2)
    gl = ax.gridlines(draw_labels=True, linewidth=0.4, color="#aaaaaa", alpha=0.8, linestyle="--", zorder=2); gl.top_labels = gl.right_labels = False
