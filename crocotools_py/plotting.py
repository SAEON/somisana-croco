...
def plot(fname,
    ...
    if len(time_var)==1: # single time-step specified, so a plot, not an animation
        ...
        def plot_tstep(i):
            ...
            # the data always needs updating, regardless of whether we're
            # labelling the time (this used to be nested under the if above,
            # which meant animations with add_time_label=False never advanced)
