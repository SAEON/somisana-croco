...
%  make_CFSR_hourly.m
% 
%  script is based on make_ECMWF.m as part of croco_tools, adapted to
%  process data downloaded from https://rda.ucar.edu/#!lfd?nb=y&b=proj&v=
%  NCEP%20Climate%20Forecast%20System%20Reanalysis
% 
%  Further Information:  
...
if Download_data==1
  ...
  disp(['===================='])
  disp('Get CFSR data from my DOWNLOADED data')
  disp(['===================='])

  download_CFSR_hourly(Ymin,Ymax,Mmin,Mmax,lonmin,lonmax,latmin,latmax,...
   ...
   for Y=Ymin:Ymax
    ...
    if Y==Ymax
     ...
     for M=mo_min:mo_max
      ...
      %
      %-------------------------------------------------------------------%
      nc=netcdf([NCEP_dir,'TMP_L1_Y',num2str(Y),'M',num2str(M),'.nc']);
      CFSR_time=nc{'time'}(:);
      close(nc);
      ...
      %
	  fname =   [NCEP_dir,'TMP_L1_Y',num2str(Ym),'M',num2str(Mm),'.nc'];
      %
      disp(' ')
      disp('======================================================')
      ...
      for aa=1:itolap_ncep
	    aa0=itolap_ncep-aa; 
	    interp_CFSR_hourly(NCEP_dir,Ym,Mm,Roa,interp_method,lon1,lat1,...
      ...
      for tndx=1:tlen0
	    ...
	    interp_CFSR_hourly(NCEP_dir,Y,M,Roa,interp_method,lon1,lat1,...
      ...
      fname=[NCEP_dir,'TP_Y',num2str(Yp),'M',num2str(Mp),'.nc'];
	...
	%
      ...
      for tndx=tlen0+itolap_ncep+1:tlen;
        ...
        interp_CFSR_hourly(NCEP_dir,Yp,Mp,Roa,interp_method,lon1,lat1,...
