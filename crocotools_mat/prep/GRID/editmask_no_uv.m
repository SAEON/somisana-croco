function editmask_no_uv(grid_file,coast_file);
%
% script created by Giles Fearon, adapted from existing CROCOTOOLS scripts
% This is exactly the same function as editmask.m in croco_tools, only the
% call to write_mask has been changed to write_mask_no_uv
%
...
%-----------------------------------------------------------------------
  ...
  case 'save',
    ...
    mask=rmask;
    write_mask_no_uv(mfile,rmask,umask,vmask,pmask);
    changed=0;
    editmask refresh;
