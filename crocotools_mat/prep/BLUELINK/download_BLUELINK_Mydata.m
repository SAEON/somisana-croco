function download_BLUELINK_Mydata(Ymin,Ymax,Mmin,Mmax,lonmin,lonmax,latmin,latmax,...
                       OGCM_dir,OGCM_prefix,path,Yorig)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% script created by Giles Fearon, adapted from existing CROCOTOOLS scripts
% Extract a subgrid from downloaded BLUELINK data to get a ROMS forcing
...
% Loop on the months
  ...
  for M=mo_min:mo_max
    ...
    % use the temperature file to get the generic time and grid data
    bluelinkfile_t=[path,'temp_',num2str(Y),'_',num2str(M),'.nc'];
    % model is an Arakawa B grid, so u and v are on a separate grid
    bluelinkfile_u=[path,'u_',num2str(Y),'_',num2str(M),'.nc'];
    
    %
    % Find a subset of the BLUELINK grid
    %
    [i1min,i1max,i2min,i2max,i3min,i3max,jrange,krange,lonT,latT,lonUV,latUV,depth]=...
     get_BLUELINK_subgrid_Mydata(bluelinkfile_t,bluelinkfile_u,lonmin,lonmax,latmin,latmax);
    %
    % Get BLUELINK time 
    %
    nc=netcdf(bluelinkfile_t);
    time=nc{'Time'}(:);
    nc(close);
    Nt=length(time);
    time_origin=ncreadatt(bluelinkfile_t,'Time','units');
    ...
    BLUELINK_time=time-datenum(Yorig,1,1);
    
    trange=1:Nt;
%
% Extract HYCOM data
%
    % NOTE 'bluelinkfile' changed to 'path' in the call to the function -
    % we need to edit extract_BLUELINK_Mydata() accordingly
    extract_BLUELINK_Mydata(OGCM_dir,OGCM_prefix,path,Y,M,...
                 lonT,latT,lonUV,latUV,depth,BLUELINK_time,...
