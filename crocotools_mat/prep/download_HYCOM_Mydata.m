function download_HYCOM_Mydata(Ymin,Ymax,Mmin,Mmax,lonmin,lonmax,latmin,latmax,...
...
% Loop on the months
  ...
  for M=mo_min:mo_max
    ...
    hycomfile=[path,num2str(Y),'_',num2str(M),'.nc'];
    ...
    Nt=length(time);
    time_origin=ncreadatt(hycomfile,'time','units');
    ...
    HYCOM_time=time-datenum(Yorig,1,1);
    
    trange=1:Nt;
%
% Extract HYCOM data
%
    extract_HYCOM_Mydata(OGCM_dir,OGCM_prefix,hycomfile,Y,M,...
                 lon,lat,depth,HYCOM_time,...
