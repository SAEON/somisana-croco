function download_CFSRV2_hourly(Ymin,Ymax,Mmin,Mmax,lonmin,lonmax,latmin,latmax,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ifile=[ncep_url,'cdas1.',sprintf('%04d',Ymin),sprintf('%02d',Mmin),'01.sfluxgrbf.grb2.nc'];
...
for k=1:length(vnames)
    ...
    disp(['=========================='])
    vname=char(vnames(k));
    if k==1 % Dealing with the mask (% WHY IS THE LAND MASK NEEDED FOR ATMOSPHERIC FORCING? ... to avoid using land points for interpolation? But excluding land points means the dropoff would be underestimated?)
        %
        disp(['==========================']);
...
%
      ...
      for M=mo_min:mo_max
       ...
       extract_CFSRV2_hourly(NCEP_dir,ncep_url,...
