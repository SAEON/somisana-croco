function extract_HYCOM_Mydata(HYCOM_dir,HYCOM_prefix,hycomfile,year,month,...
                      lon,lat,depth,time,...
...
% Extract a subset from a local HYCOM file
...
%
ssh=nc{'surf_el'}(:);
scale = ncreadatt(hycomfile,'surf_el','scale_factor');
offset = ncreadatt(hycomfile,'surf_el','add_offset');
missingval = ncreadatt(hycomfile,'surf_el','missing_value');
if missingval<0
...
u=nc{'water_u'}(:);
scale = ncreadatt(hycomfile,'water_u','scale_factor');
offset = ncreadatt(hycomfile,'water_u','add_offset');
missingval = ncreadatt(hycomfile,'water_u','missing_value');
if missingval<0
...
disp('    ...V')
v=nc{'water_v'}(:);
scale = ncreadatt(hycomfile,'water_v','scale_factor');
offset = ncreadatt(hycomfile,'water_v','add_offset');
missingval = ncreadatt(hycomfile,'water_v','missing_value');
if missingval<0
...
disp('    ...TEMP')
temp=nc{'water_temp'}(:);
scale = ncreadatt(hycomfile,'water_temp','scale_factor');
offset = ncreadatt(hycomfile,'water_temp','add_offset');
missingval = ncreadatt(hycomfile,'water_temp','missing_value');
if missingval<0
...
disp('    ...SALT')
salt=nc{'salinity'}(:);
scale = ncreadatt(hycomfile,'salinity','scale_factor');
offset = ncreadatt(hycomfile,'salinity','add_offset');
missingval = ncreadatt(hycomfile,'salinity','missing_value');
if missingval<0
...
%
create_HYCOM_Mydata([HYCOM_dir,HYCOM_prefix,'Y',num2str(year),'M',num2str(month),'.cdf'],...
