    ...
    def run_cmd(c, run_date, start_date, end_date, domain, outputDir):
        ...
        if os.path.exists(f) == False:
            for i in range(MAX_RETRIES):
                ...
                try:
                    result = subprocess.run(runcommand, shell=True, check=True, capture_output=True)  
...
def download_mercator_async(usrname, passwd, domain, run_date, hdays, fdays, outputDir):
    """
    Download the operational Mercator ocean output
