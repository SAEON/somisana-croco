function [time,data_ts]=get_station_ts(dirname,sta_no,level,var,origin_datenum,time_start,time_end)
...
%     
    if count==1
        ...
        data=cat(1,data,sta_var);
        depth=cat(1,depth,sta_depth);
    end
...
disp(['model bottom layer depth at station = ',num2str(mean(depth(:,1))),' m']);

if level>=1
    data_ts=data(:,level);
    disp(['model selected layer depth at station = ',num2str(mean(depth(:,level))),' m']);
else
    data_ts=nan(length(time),1);
    for t=1:length(time)
        data_ts(t)=interp1(depth(t,:),data(t,:),level);
    end
end
