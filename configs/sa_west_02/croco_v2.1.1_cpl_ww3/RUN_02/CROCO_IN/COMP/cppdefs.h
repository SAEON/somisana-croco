...
# define  OW_COUPLING
# ifdef OW_COUPLING
#  define OW_COUPLING_FULL
#  undef WAVE_SMFLUX
/*
!  RUN_02 EXPERIMENT DEFINITION - "conservative wave effects only"
!
!  WCI_NO_WAVE_MIXING requires the patched gls_mixing.F (see
!  ../../HPC_SOURCE_PATCH.md). It switches the three wave-mixing blocks in the
!  GLS surface boundary conditions back to their standard non-wave branches:
!  no breaking-TKE injection (flux_top=0) and Charnock instead of wave-height
!  surface roughness. Everything else under MRL_WCI - Stokes drift, vortex
!  force, Stokes-Coriolis, Bernoulli head - stays fully active.
!
!  The bottom-side wave keys (BBL, WAVE_STREAMING, WAVE_FRICTION) are turned
!  off further down, so bottom friction is the plain log-law with Zob=1e-3,
!  identical to the no-wave reference run croco_v2.1.1/C10_I01_GLORYS_ERA5.
!  RUN_02 minus no-wave is therefore a pure conservative-wave signal.
!
!  NOTE: the patch is inert unless this key is defined, so RUN_01 is
!  unaffected by it and needs no recompile.
*/
#  define WCI_NO_WAVE_MIXING
/*
!  WCI_NO_WAVE_STREAMING requires the patched cppdefs_dev.h (see
!  ../../HPC_SOURCE_PATCH.md). cppdefs_dev.h:81 force-defines WAVE_STREAMING
!  whenever OW_COUPLING && MRL_WCI, and because it is included at the END of
!  this file (via set_global_definitions.h) it silently overrides the
!  `#undef WAVE_STREAMING` further down. Without the patch this run would
!  still carry the bottom-streaming body force - and would compile and run
!  without complaint.
*/
#  define WCI_NO_WAVE_STREAMING
# endif
...
# ifdef OW_COUPLING
#  define MRL_WCI
/*  RUN_02: BBL off - no wave-current bottom stress (also avoids any
!   dependence on the patched bbl.F). Bottom stress comes from the log-law. */
#  undef  BBL
# endif
# ifdef MRL_WCI
#  ifndef OW_COUPLING
...
#  endif
#  undef  WAVE_ROLLER
/*  RUN_02: bottom streaming and wave-enhanced bottom roughness both off.
!   Dropping BOTH is what matters - it skips the `#define Zob Zob_my`
!   override at mrl_wci.F:173, so get_vbc uses the same Zob=1e-3 log-law as
!   the no-wave run. Turning off only one of them leaves the override active. */
