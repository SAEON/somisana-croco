     ...
     &        start(2), count(2), ibuff(6),   nf_fread, checkdims
      ...
      if (ierr .eq. nf_noerr) then
       ...
       if (mynode.eq.0) write(stdout,'(/1x,2A,I4,1x,A/12x,A,I4,1x,3A/)')
     ...
     &                                        ininame(1:lstr), '''.'
        ...
        if (mynode.eq.0) write(stdout,'(/1x,4A/12x,A/)')
     &               '### ERROR: get_initial :: ',
        ...
        if (ierr .eq. nf_noerr) then
          ...
          if (ierr .eq. nf_noerr) then
            ...
            elseif (units(1:3).eq.'day') then
              ...
              if (mynode.eq.0) write(stdout,'(/1x,4A/8x,3A/)')
     &              'GET_INITIAL ',
     ...
     &                   'in netCDF file ''', ininame(1:lstr), '''.'
            ...
            if (mynode.eq.0) write (stdout,'(/1x,2A/8x,5A/)')
     &             'GET_INITIAL ERROR: ',
     ...
     &             ''' in netCDF file ''',  ininame(1:lstr), '''.'
          ...
          if (mynode.eq.0) write(stdout,2) vname(1,indxTime)(1:lvar)
     &                                , record, ininame(1:lstr)
     &                                , nf_strerror(ierr)
          goto 99
        endif
      else
        if (mynode.eq.0) write(stdout,1) vname(1,indxTime)(1:lvar)
     &                                , ininame(1:lstr)
     &                                , nf_strerror(ierr)
        goto 99
      endif
     ...
     &         'read time and record indices.'
        ...
        nrechis=0
        nrecavg=0
        if (mynode.eq.0) write(stdout,'(/1x,2A,G12.4,1x,A,I4)')
     &          'GET_INITIAL -- ',
     ...
     &                                         record, r2dvar)
        if (ierr .ne. nf_noerr) then
          if (mynode.eq.0) write(stdout,2) vname(1,indxZ)(1:lvar)
     &                                ,record,ininame(1:lstr)
          goto 99
        endif
      else
        if (mynode.eq.0)  write(stdout,1) vname(1,indxZ)(1:lvar)
     &                                ,ininame(1:lstr)
        goto 99
      endif
     ...
     &                                         record, u2dvar)
        if (ierr .ne. nf_noerr) then
          if (mynode.eq.0) write(stdout,2) vname(1,indxUb)(1:lvar)
     &                                  , record, ininame(1:lstr)
          goto 99
        endif
      else
        if (mynode.eq.0) write(stdout,1) vname(1,indxUb)(1:lvar)
     &                                ,ininame(1:lstr)
        goto 99
      endif
     ...
     &                                         record, v2dvar)
        if (ierr .ne. nf_noerr) then
          if (mynode.eq.0) write(stdout,2) vname(1,indxVb)(1:lvar)
     &                                , record, ininame(1:lstr)
          goto 99
        endif
      else
        if (mynode.eq.0) write(stdout,1) vname(1,indxVb)(1:lvar)
     &                                ,ininame(1:lstr)
        goto 99
      endif
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (u(-1,-1,1,tindx), ncid, varid,
     &                                         record, u3dvar)
        if (ierr .ne. nf_noerr) then
          if (mynode.eq.0) write(stdout,2) vname(1,indxU)(1:lvar)
     &                                 , record, ininame(1:lstr)
          goto 99
        endif
      else
        if (mynode.eq.0) write(stdout,1) vname(1,indxU)(1:lvar)
     &                                ,ininame(1:lstr)
        goto 99
      endif
      ...
      if (ierr .eq. nf_noerr) then
        ierr=nf_fread (v(-1,-1,1,tindx), ncid, varid,
     &                                         record, v3dvar)
        if (ierr .ne. nf_noerr) then
          if (mynode.eq.0) write(stdout,2) vname(1,indxV)(1:lvar)
     &                                 , record, ininame(1:lstr)
          goto 99
        endif
      else
        if (mynode.eq.0) write(stdout,1) vname(1,indxV)(1:lvar)
     &                                ,ininame(1:lstr)
        goto 99
      endif
      do itrc=1,NT
        ...
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (t(-1,-1,1,tindx,itrc), ncid, varid,
     &                                                record, r3dvar)
          if (ierr .ne. nf_noerr) then
            if (mynode.eq.0) write(stdout,2) vname(1,indxV+itrc)(1:lvar)
     &                                        , record, ininame(1:lstr)
            goto 99
          endif
        else
          if (mynode.eq.0) write(stdout,3) vname(1,indxV+itrc)(1:lvar),
     &                                    ininame(1:lstr)
          got_tini(itrc)=.false.
        endif
      enddo
        ...
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (trb(-1,-1,0,tindx,itke), ncid,
     &                                    varid, record, w3dvar)
          if (ierr .ne. nf_noerr) then
     ...
     &                                     , record, ininame(1:lstr)
          ...
          if (mynode.eq.0) write(stdout,4) vname(1,indxTke)(1:lvar)
     &                                  , ininame(1:lstr)
      endif
        lvar=lenstr(vname(1,indxGls))
        ierr=nf_inq_varid (ncid, vname(1,indxGls)(1:lvar), varid)
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (trb(-1,-1,0,tindx,igls), ncid,
     &                                    varid, record, w3dvar)
          if (ierr .ne. nf_noerr) then
     ...
     &                                     , record, ininame(1:lstr)
          ...
          if (mynode.eq.0) write(stdout,4) vname(1,indxGls)(1:lvar)
     &                                  , ininame(1:lstr)
      endif
        lvar=lenstr(vname(1,indxAkv))
        ierr=nf_inq_varid (ncid, vname(1,indxAkv)(1:lvar), varid)
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (Akv(-1,-1,0), ncid,
     &                                    varid, record, w3dvar)
          if (ierr .ne. nf_noerr) then
            if (mynode.eq.0) write(stdout,2) vname(1,indxAkv)(1:lvar)
     &                                     , record, ininame(1:lstr)
            goto 99
          endif
        else
          if (mynode.eq.0) write(stdout,4) vname(1,indxAkv)(1:lvar)
     &                                  , ininame(1:lstr)
        endif
        lvar=lenstr(vname(1,indxAkt))
        ierr=nf_inq_varid (ncid, vname(1,indxAkt)(1:lvar), varid)
       if (ierr .eq. nf_noerr) then
          ierr=nf_fread (Akt(-1,-1,0,itemp), ncid,
     &                                    varid, record, w3dvar)
          if (ierr .ne. nf_noerr) then
            if (mynode.eq.0) write(stdout,2) vname(1,indxAkt)(1:lvar)
     &                                     , record, ininame(1:lstr)
            goto 99
          endif
        else
          if (mynode.eq.0) write(stdout,4) vname(1,indxAkt)(1:lvar)
     &                                  , ininame(1:lstr)
        endif
        lvar=lenstr(vname(1,indxAks))
        ierr=nf_inq_varid (ncid, vname(1,indxAks)(1:lvar), varid)
        if (ierr .eq. nf_noerr) then
          ierr=nf_fread (Akt(-1,-1,0,isalt), ncid,
     &                                    varid, record, w3dvar)
          if (ierr .ne. nf_noerr) then
            if (mynode.eq.0) write(stdout,2) vname(1,indxAks)(1:lvar)
     &                                     , record, ininame(1:lstr)
            goto 99
          endif
        else
          if (mynode.eq.0) write(stdout,4) vname(1,indxAks)(1:lvar)
     &                                  , ininame(1:lstr)
        endif
      ierr=nf_close(ncid)
