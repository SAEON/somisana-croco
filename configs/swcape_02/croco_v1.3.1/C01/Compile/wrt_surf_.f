...
!$AGRIF_END_DO_NOT_TREAT
      integer*4 tile,itrc,i,j,k,ivar
     ...
     &                 1, MPI_COMM_WORLD, status, ierr)
      ...
      call def_surf (ncidsurf, nrecsurf, ierr)
      if (ierr .ne. nf_noerr) goto 99
      lstr=lenstr(surfname)
      if (iic.eq.0) nrecsurf=nrecsurf+1
      if (nrpfsurf.eq.0) then
        record=nrecsurf
      else
        record=1+mod(nrecsurf-1, nrpfsurf)
      endif
      type=filetype_surf
      ibuff(1)=iic
      ibuff(2)=nrecrst
      ...
      count(1)=4
      count(2)=1
      ierr=nf_put_vara_int (ncidsurf, surfTstep,
     &                                            start, count, ibuff)
     ...
     &                  ,' mynode =', mynode
      ...
      ierr=nf_put_var1_double (ncidsurf, surfTime, record, time
     &  )
      if (ierr .ne. nf_noerr) then
     ...
     &                  ,' mynode =', mynode
      ...
      ierr=nf_put_var1_double (ncidsurf, surfTime2, record, time
     &  )
      if (ierr .ne. nf_noerr) then
     ...
     &                  ,' mynode =', mynode
        ...
        itrc = 1
          if (wrtsurf(itrc)) then
           work2d=t(:,:,N,nstp,1)
           call fillvalue2d(work2d,ncidsurf,surf_surft(itrc),
     &                      indxsurft+itrc-1,
     &                      record,r2dvar,type)
           work2d=t(:,:,N,nstp,2)
           call fillvalue2d(work2d,ncidsurf,surf_surfs(itrc),
     &                      indxsurfs+itrc-1,
     &                      record,r2dvar,type)
           work2d=zeta(:,:,knew)
           call fillvalue2d(work2d,ncidsurf,surf_surfz(itrc),
     &                      indxsurfz+itrc-1,
     &                      record,r2dvar,type)
           work2d=u(:,:,N,nstp)
           call fillvalue2d(work2d,ncidsurf,surf_surfu(itrc),
     &                      indxsurfu+itrc-1,
     &                      record,u2dvar,type)
           work2d=v(:,:,N,nstp)
           call fillvalue2d(work2d,ncidsurf,surf_surfv(itrc),
     &                      indxsurfv+itrc-1,
     &                      record,v2dvar,type)
        endif
  1   format(/1x,'WRT_surf ERROR while writing variable ''', A,
     &          ''' into diag file.', /11x, 'Time record:',
     &                   I6,3x,'netCDF error code',i4,3x,a,i4)
      goto 100
  99  may_day_flag=3
 100  continue
      ierr=nf_close (ncidsurf)
      if (nrpfsurf.gt.0 .and. record.ge.nrpfsurf) ncidsurf=-1
      if (ierr .eq. nf_noerr) then
        if (mynode.eq.0) write(stdout,'(6x,A,2(A,I4,1x),A,I3)')
     &            'WRT_surf -- wrote',
     &            ' diag fields into time record =', record, '/',
     &             nrecsurf  ,' mynode =', mynode
      else
        if (mynode.eq.0) write(stdout,'(/1x,2A/)')
     &             'WRT_surf ERROR: Cannot ',
