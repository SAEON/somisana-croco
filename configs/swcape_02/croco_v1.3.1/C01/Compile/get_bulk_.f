  ...
  1   i=3-itbulk
     ...
     &          bulk_time(itbulk)=bulk_time(i)
        if (ltairgrd.eq.1) then
          ierr=nf_fread (tairg(-1,-1,i), ncidbulk, tair_id,
     &                                            bulk_rec,  r2dvar)
        else
        ...
        if (lrhumgrd.eq.1) then
          ierr=nf_fread (rhumg(-1,-1,i), ncidbulk, rhum_id,
     &                                            bulk_rec,  r2dvar)
        else
        ...
        if (lradlwgrd.eq.1) then
          ierr=nf_fread (radlwg(-1,-1,i), ncidbulk, radlw_id,
     &                                            bulk_rec,  r2dvar)
        else
        ...
        if (lradswgrd.eq.1) then
          ierr=nf_fread (radswg(-1,-1,i), ncidbulk, radsw_id,
     &                                            bulk_rec,  r2dvar)
        else
        ...
        if (lprategrd.eq.1) then
          ierr=nf_fread (prateg(-1,-1,i), ncidbulk, prate_id,
     &                                            bulk_rec,  r2dvar)
        else
        ...
        if (luwndgrd.eq.1) then
          ierr=nf_fread(uwndg(-1,-1,i), ncidbulk, uwnd_id,
     &                                              bulk_rec, u2dvar)
        else
        ...
        if (lvwndgrd.eq.1) then
          ierr=nf_fread(vwndg(-1,-1,i), ncidbulk, vwnd_id,
     &                                              bulk_rec, v2dvar)
        else
       ...
       itbulk=i
       if (mynode.eq.0) write(stdout,'(6x,A,1x,A,1x,F10.4,1x,I4)')
     &              'GET_BULK   --',
  ...
  99  may_day_flag=2
     ...
     &                         0.25D0*(vwnd(i,j)+vwnd(i,j+1))**2)
                cff      = 1.D0-swparam
                uwnd_r   = 0.5D0*( uwnd(i+1,j)+uwnd(i,j)
     &                   - cff*( u(i+1,j,N,nrhs)+u(i,j,N,nrhs))
     &                   )
                vwnd_r   = 0.5D0*( vwnd(i,j+1)+vwnd(i,j)
