     ...
     &          , isalt
      ...
      integer*4 :: lbx,ubx,lby,uby
      integer*4 chunk_size_X,margin_X,chunk_size_E,margin_E
      integer*4 Istrmpi,Iendmpi,Jstrmpi,Jendmpi, i_X,j_E
      integer*4,dimension(0:NP_XI-1,0:NP_ETA-1)  ::   myproc
      real,dimension(0:LLm+1,0:MMm+1)  ::   zmask
      integer*4,dimension(0:NNODES-1) :: mymynode
      integer*4,dimension(0:NNODES-1) :: mymynode2
      integer*4 ncid,varid,nerr
      integer*4 ip1,im1,jp1,jm1,inu2
      integer*4 Npts
      parameter (Npts=2)
...
!$AGRIF_END_DO_NOT_TREAT
      ...
      if (nsize.ne.NNODES) then
        if (mynode.eq.0) write(stdout,'(/1x,A,I5,1x,A,I5,A/)')
